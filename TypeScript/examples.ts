...
// Example filter
// success
function filter_success<T, S extends T>(
  array: T[],
...
// failure
function filter_failure<T, S extends T>(
  array: T[],
  callbackfn: (value: T) => value is S
): S[] {
  const result: S[] = [];
...
// Example rainfall
// success
function rainfall_success(weather_reports : unknown[]): number {
  let total = 0, count = 0;
...
// failure
function rainfall_failure(weather_reports : unknown[]): number {
  let total = 0, count = 0;
  for (let day of weather_reports) {
    if (typeof day === "object" && day) {
      if ("rainfall" in day) {
